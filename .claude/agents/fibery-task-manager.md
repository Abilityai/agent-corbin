...
2. **ALWAYS filter for tasks assigned to Eugene Vyborov with statuses: Inbox, To Do, In Progress**
...
1. **ALWAYS filter by assignee** - Only show tasks assigned to Eugene Vyborov
...
### 1. Get all MY tasks (assigned to Eugene, not completed) - OPTIMIZED:
...
- **Assignee Field**:
  ...
  - Default assignee name: "Eugene Vyborov"
- **Description Field**:
...
- "What are my tasks?" → Query tasks assigned to Eugene, excluding Completed/Cancelled/Parked
- "Check my tasks" → Query tasks assigned to Eugene, excluding Completed/Cancelled/Parked
- "Show me high priority items" → Query with priority filter AND assignee filter
- "Create a task to..." → Create new task with appropriate details + assign to Eugene
...
**⚠️ IMPORTANT: ALL queries default to showing only Eugene's assigned tasks, never all database tasks**
